...
object Routing {
  ...
  private def tryParseConf(confAsStr: String) = {
    ...
    Try {
      ...
      val stock = Stocks.getById(request.stock).getOrElse(throw new RuntimeException("invalid stockId"))
    ...
    Flow[Message].flatMapConcat {
      case TextMessage.Strict(confAsStr) =>
        tryParseConf(confAsStr) match {
          case Success(conf) =>
            val simulation = simulationFactory(conf).map(TextMessage.apply)
            log.debug(s"Trade simulation for $conf has been created")
            simulation

          case Failure(thr) => Source.single(TextMessage(s"Can't extract simulation conf from '$confAsStr' because:\n${thr.getMessage}"))
        }
